...
with tab8:
    ...
    if not df_ops.empty:
        ...
        with h_col1:
            h_sinf_opts = ["الكل", "🏠 DRNI"] + sorted(set(["FN","CT","NTG","BT"] + df_ops["الصنف"].dropna().unique().tolist()))
        ...
        # ── فلتر DRNI: عرض كل المنازل وما عندهم من عمل (إخراج لم يكتمل بالاستلام)
        if h_sinf == "🏠 DRNI":
            # احسب لكل منزل+منتج+صنف: المُخرَج - المُستلَم
            df_out_d = df_ops_h[df_ops_h["النوع"]=="إخراج"].groupby(["المنزل","المنتج","الصنف"])["الكمية"].sum().reset_index()
            df_out_d.columns = ["المنزل","المنتج","الصنف","المُخرَج"]
            df_in_d = df_ops_h[df_ops_h["النوع"]=="استلام"].groupby(["المنزل","المنتج","الصنف"])["الكمية"].sum().reset_index()
            df_in_d.columns = ["المنزل","المنتج","الصنف","المُستلَم"]
            df_drni = df_out_d.merge(df_in_d, on=["المنزل","المنتج","الصنف"], how="left").fillna(0)
            df_drni["المُخرَج"] = df_drni["المُخرَج"].astype(int)
            df_drni["المُستلَم"] = df_drni["المُستلَم"].astype(int)
            df_drni["الباقي"] = df_drni["المُخرَج"] - df_drni["المُستلَم"]
            # فقط المنازل التي عندها عمل لم يُستلم بعد
            df_drni = df_drni[df_drni["الباقي"] > 0].reset_index(drop=True)
            # أضف تاريخ آخر إخراج لكل منزل+منتج+صنف
            df_last_date = df_ops_h[df_ops_h["النوع"]=="إخراج"].sort_values("التاريخ").groupby(["المنزل","المنتج","الصنف"])["التاريخ"].last().reset_index()
            df_drni = df_drni.merge(df_last_date, on=["المنزل","المنتج","الصنف"], how="left")
            df_drni = df_drni.sort_values("التاريخ", ascending=False).reset_index(drop=True)
            df_drni = df_drni[["التاريخ","المنزل","المنتج","الصنف","المُخرَج","المُستلَم","الباقي"]]

            if df_drni.empty:
                st.success("✅ كل المنازل سلّمت عملها!")
            else:
                st.markdown(f"#### 🏠 DRNI — {len(df_drni['المنزل'].unique())} منزل عندهم عمل")
                import json as _json
                cols_d = list(df_drni.columns)
                rows_d = df_drni.values.tolist()
                cols_json_d = _json.dumps(cols_d, ensure_ascii=False)
                rows_json_d = _json.dumps([[str(v) for v in r] for r in rows_d], ensure_ascii=False)
                total_d = len(df_drni)
                drni_html = f"""<!DOCTYPE html>
<html dir="rtl">
<head><meta charset="utf-8">
<style>
  body {{ margin:0; background:#0e0e1a; color:white; font-family:Arial,sans-serif; font-size:13px; }}
  .toolbar {{ padding:8px; background:#0e0e1a; position:sticky; top:0; z-index:10; display:flex; gap:8px; align-items:center; }}
  .btn {{ background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.25); border-radius:8px; padding:7px 16px; color:white; font-size:15px; cursor:pointer; }}
  .count {{ color:#aaa; font-size:12px; }}
  .table-wrap {{ overflow:auto; max-height:360px; }}
  table {{ width:100%; border-collapse:collapse; }}
  th {{ background:#1a1a2e; color:#e0b060; padding:8px 10px; text-align:center; position:sticky; top:0; white-space:nowrap; cursor:pointer; user-select:none; }}
  ...
  tr:hover td {{ background:rgba(255,200,50,0.07); }}
...
<div class="toolbar">
  ...
  <span class="count">🏠 {total_d} صف — منازل عندها عمل لم يُستلم</span>
</div>
...
<script>
var COLS = {cols_json_d};
var ROWS = {rows_json_d};
var sortCol = 0;
var sortAsc = false;
...
function renderTable() {{
  ...
  var sorted = ROWS.slice().sort(function(a,b) {{
    ...
    if (da !== null && db !== null) return sortAsc ? da - db : db - da;
...
renderTable();
                ...
                components.html(drni_html, height=440, scrolling=False)
                st.caption(f"📊 إجمالي: {total_d} صف")
            st.divider()
