...
renderTable();
            ...
            # ── تصدير PDF
            try:
                pdf_bytes = generate_history_pdf(df_sum, title="سجل History - Baby Sympa")
                st.download_button(
                    label="📄 تصدير الجدول PDF",
                    data=pdf_bytes,
                    file_name=f"history_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
                    mime="application/pdf",
                    use_container_width=True,
                    key="pdf_export_hist"
                )
            except Exception as e:
                st.warning(f"⚠️ تعذّر توليد PDF: {e}")
