...
# ── المخزن
    ...
    if not df_ops.empty:
        ...
        if df_ops_f.empty:
            ...
            df_show = df_balance[cols_order].copy()
            df_show["_sort_dt"] = df_show["التاريخ"].apply(_arabic_date_sort_key)
            df_show = df_show.sort_values("_sort_dt", ascending=False).drop(columns=["_sort_dt"]).reset_index(drop=True)
...
# ── History
    ...
    if not df_ops.empty:
        ...
        if df_ops_h.empty:
            ...
            df_sum = df_ops_h.groupby(["التاريخ","الصنف","المنزل","المنتج","النوع"])["الكمية"].sum().reset_index()
            df_sum["_sort_dt"] = df_sum["التاريخ"].apply(_arabic_date_sort_key)
            df_sum = df_sum.sort_values("_sort_dt", ascending=False).drop(columns=["_sort_dt"]).reset_index(drop=True)
