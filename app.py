...
# باقي الدوال (load_images, load_notes, save_note, delete_note...) تبقى كما هي
...
if st.button("🔄 تحديث جميع البيانات", use_container_width=True):
    for fn in [load_produits, load_types, load_منازل, load_operations, load_ventes, load_livraisons, load_notes]:
        fn.clear()
    st.rerun()
...
# ── إخراج، استلام، البيع، المخزن، الأخطاء (تبقى كما هي)

with tab1:
    # ... (كود الإخراج كما هو)
    pass  # ضع كودك السابق هنا
...
with tab6:
    ...
    if not livs:
        ...
        for liv in livs:
            ...
            with col2:
                st.write(f"الكمية المطلوبة: **{liv['الكمية المطلوبة']}**")
            with col3:
...
# باقي الأتبويب (الصور، History، الملاحظات) ...

st.caption("Baby Sympa - لوحة تحكم الورشة")
