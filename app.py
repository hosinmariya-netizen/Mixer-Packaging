...
# ── زر رجوع ثابت في أعلى الشاشة + نافذة تأكيد خروج مخصصة
# ملاحظة: بعض المتصفحات/التطبيقات على الموبايل (خصوصاً عند إضافة الصفحة للشاشة
# الرئيسية) لا تدعم نافذة confirm() الأصلية للمتصفح فتتجاهلها بصمت. لذلك نستخدم
# نافذة تأكيد مبنية بـ HTML/CSS عادية بدل الاعتماد على confirm().
st.markdown("""
<style>
.back-btn-fixed {
    position: fixed;
...
<button class="back-btn-fixed" onclick="document.getElementById('exit-modal').classList.add('show')">⬅️</button>

<div class="exit-modal-overlay" id="exit-modal">
  <div class="exit-modal-box">
    <p>⚠️ هل تريد الخروج؟<br>تأكد من أن التعديلات محفوظة أولاً بالضغط على زر 💾 الحفظ.</p>
    <div class="exit-modal-buttons">
      <button class="exit-btn-cancel" onclick="document.getElementById('exit-modal').classList.remove('show')">إلغاء</button>
      <button class="exit-btn-confirm" onclick="window.history.back()">خروج</button>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
if st.button("🔄 تحديث جميع البيانات", use_container_width=True):
    for fn in [load_produits, load_types, load_منازل, load_operations, load_livraisons, load_sheet_csv]:
        fn.clear()
    st.rerun()

# ── زر الحفظ: تأكيد أن كل التعديلات مرفوعة ومحفوظة في Google Sheets قبل الخروج
if st.button("💾 حفظ", use_container_width=True, key="save_confirm_btn"):
    for fn in [load_produits, load_types, load_منازل, load_operations, load_livraisons, load_sheet_csv]:
        fn.clear()
    st.success("✅ تم حفظ جميع التعديلات في Google Sheets. يمكنك الخروج الآن بأمان.")

TAB_NAMES = [
    "📤 إخراج", "🏪 المخزن", "📦 No Livraison", "📊 History"
]
...
# ── المخزن
    ...
    if not df_ops.empty:
        ...
        if df_ops_f.empty:
            ...
            cols_order = ["التاريخ","المنزل","المنتج","الصنف","المُخرَج","المُستلَم"]
            ...
            # ── سقف 300 نتيجة لتخفيف الحمل على الاستجابة
            MAX_MAKHZAN_SHOWN = 300
            total_makhzan = len(df_show)
            if total_makhzan > MAX_MAKHZAN_SHOWN:
                st.caption(f"🔎 تُعرض أحدث {MAX_MAKHZAN_SHOWN} نتيجة من أصل {total_makhzan} — ضيّق البحث لعرض نتائج أدق.")
                df_show = df_show.head(MAX_MAKHZAN_SHOWN)
...
<tbody>{rows_html}</tbody>
            ...
            st.caption(f"📊 إجمالي السجلات: {total_makhzan}")
    else:
        st.info("المخزن فارغ.")
    ...
    if not df_ops_liv.empty and "المنزل" in df_ops_liv.columns:
        ...
        if not df_with_manzil.empty:
            ...
            if not df_last.empty:
                ...
                # ── سقف 300 نتيجة لتخفيف الحمل على الاستجابة
                MAX_LIV_SHOWN = 300
                total_liv_display = len(df_display)
                if total_liv_display > MAX_LIV_SHOWN:
                    st.caption(f"🔎 تُعرض {MAX_LIV_SHOWN} نتيجة من أصل {total_liv_display}.")
                    df_display = df_display.head(MAX_LIV_SHOWN)
...
# ── History
    ...
    if not df_ops.empty:
        ...
        if df_ops_h.empty:
            ...
            # ── سقف 300 نتيجة لتخفيف الحمل على الاستجابة
            MAX_SUMMARY_SHOWN = 300
            total_summary = len(df_sum)
            if total_summary > MAX_SUMMARY_SHOWN:
                st.caption(f"🔎 تُعرض أحدث {MAX_SUMMARY_SHOWN} نتيجة من أصل {total_summary} — ضيّق البحث لعرض نتائج أدق.")
                df_sum = df_sum.head(MAX_SUMMARY_SHOWN)
