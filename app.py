...
def save_vente(تاريخ, منزل, منتج, صنف, كمية):
    try:
        ws = get_worksheet("البيع")
        if not ws.get_all_values():
            ws.append_row(["التاريخ","النوع","المنزل","المنتج","الصنف","الكمية","السجل"])
        سجل = f"بيع... {منزل} / {منتج}/{صنف}/{كمية}"
...
# ── دالة مساعدة لعرض السجلات مع بحث + اقتراحات + حذف
def show_records_with_delete(df, tab_key, delete_fn, منازل_list, produits_list):
    ...
    if s_منزل:
        ...
        if sugg:
            chosen = st.selectbox("📌 اختر من الاقتراحات:", ["—"] + sugg[:6], key=f"sugg_منزل_{tab_key}")
            if chosen != "—":
                s_منزل = chosen
    if s_منتج:
        sugg = [p for p in produits_list if s_منتج.strip().lower() in p.lower()]
        if sugg:
            chosen = st.selectbox("📌 اختر من الاقتراحات:", ["—"] + sugg[:6], key=f"sugg_منتج_{tab_key}")
            if chosen != "—":
                s_منتج = chosen
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
    "📤 إخراج", "📥 استلام", "🛒 البيع", "🏪 المخزن", "❌ الأخطاء", "📦 No Livraison", "🖼️ الصور", "📊 History"
])
...
with tab3:
    ...
    if not df_ventes.empty:
        show_records_with_delete(df_ventes, "vente", delete_vente, منازل, produits)
    else:
...
with tab6:
    ...
    if livs:
        for liv in livs:
            ...
            with col4:
                if st.button("🗑️", key=f"cancel_{liv['row_idx']}"):
...
with tab8:
    ...
    df_ops = get_df_ops()
    if not df_ops.empty:
        show_records_with_delete(df_ops, "hist", delete_operation, منازل, produits)
    else:
