...
def save_note(tar, montaj, malahaza, img_b64, img_name):
    try:
        ...
        # التأكد أن الصورة لا تتجاوز 40000 حرف
...
with tab1:
    ...
    with col1:
        out_search = st.text_input("🔍 ابحث عن منتج", key="out_search")
        filtered_out = [p for p in produits if out_search.lower() in p.lower()] if out_search else produits
        produit_out = st.selectbox("المنتج", filtered_out, key="out_prod")
    with col2:
...
with tab2:
    ...
    col1, col2 = st.columns(2)
    with col1:
        in_search = st.text_input("🔍 ابحث عن منتج", key="in_search")
        filtered_in = [p for p in produits if in_search.lower() in p.lower()] if in_search else produits
        produit_in = st.selectbox("المنتج", filtered_in, key="in_prod")
    with col2:
...
with tab3:
    ...
    col1, col2 = st.columns(2)
    with col1:
        v_search = st.text_input("🔍 ابحث عن منتج", key="v_search")
        filtered_v = [p for p in produits if v_search.lower() in p.lower()] if v_search else produits
        produit_v = st.selectbox("المنتج", filtered_v, key="v_prod")
    with col2:
...
with tab9:
    ...
    with st.expander("➕ إضافة ملاحظة جديدة", expanded=True):
        ...
        if st.button("💾 حفظ الملاحظة", use_container_width=True):
            if not note_text.strip() and not img_b64:
                st.warning("⚠️ أدخل ملاحظة أو أضف صورة على الأقل.")
            else:
                dt = datetime.combine(note_date, note_time)
                tar = format_date_arabic(dt)
                montaj_note = "" if note_prod == "— بدون منتج —" else note_prod
                if save_note(tar, montaj_note, note_text.strip(), img_b64, img_name):
