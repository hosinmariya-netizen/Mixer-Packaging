...
def save_note(tar, montaj, malahaza, img_b64, img_name, audio_b64=""):
    try:
        ...
        if len(audio_b64) > 40000:
            audio_b64 = ""
        ws.append_row([tar, montaj, malahaza, img_b64, img_name, audio_b64])
        load_notes.clear()
        return True
    except Exception as e:
...
def get_df_ops():
    ...
    if not df.empty:
        ...
        df = df.sort_values("التاريخ_dt").reset_index(drop=True)
        df["التاريخ"] = df["التاريخ_dt"].apply(lambda x: format_date_arabic(x) if pd.notna(x) else "")
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9, tab10 = st.tabs([
    ...
    "📦 No Livraison", "🖼️ الصور", "📊 History", "📝 الملاحظات", "🏠 المنازل"
])
...
# ── إدارة المنازل
with tab10:
    st.markdown("### 🏠 إضافة منزل جديد")
    st.info("سيتم إضافة الاسم تلقائياً في ورقة **السلع** (المنازل) وورقة **الكراس** (Référence)")

    new_manzil_name = st.text_input("✏️ اسم المنزل الجديد", placeholder="مثال: بباز علي", key="new_manzil_input")

    if st.button("➕ إضافة المنزل", use_container_width=True, key="add_manzil_btn"):
        if not new_manzil_name.strip():
            st.warning("⚠️ أدخل اسم المنزل أولاً.")
        else:
            added_s, added_k, errs = add_manzil(new_manzil_name.strip())
            if errs:
                for e in errs:
                    st.error("❌ " + e)
            if added_s and added_k:
                st.success("✅ تمت إضافة **" + new_manzil_name.strip() + "** في السلع والكراس معاً!")
                st.rerun()
            elif added_s:
                st.warning("⚠️ أُضيف في السلع فقط.")
            elif added_k:
                st.warning("⚠️ أُضيف في الكراس فقط.")

    st.divider()
    st.markdown("#### 📋 قائمة المنازل الحالية")
    if st.button("🔄 تحديث القائمة", use_container_width=True, key="ref_manazil"):
        load_منازل.clear()
        load_sheet_csv.clear()
        st.rerun()
    current_manazil = load_منازل()
    if current_manazil:
        for i, m in enumerate(current_manazil, 1):
            st.write(str(i) + ". " + m)
    else:
        st.info("لا توجد منازل بعد.")
