    ...
    if not df_ops.empty:
        ...
        if df_ops_h.empty:
            ...
            for _, r in df_sum.iterrows():
                rows_html += "<tr>" + "".join(
                    f"<td>{v}</td>" for v in r) + "</tr>"
            headers_html = "".join(f"<th>{c}</th>" for c in df_sum.columns)

            # ── component HTML كامل مستقل (يعمل fullscreen حقيقي)
...
<style>
  ...
  .toolbar {{ padding:8px; background:#0e0e1a; position:sticky; top:0; z-index:10; display:flex; gap:8px; }}
  .btn {{
    background:rgba(255,255,255,0.12); border:1px solid rgba(255,255,255,0.25);
    border-radius:8px; padding:7px 16px; color:white; font-size:15px;
    cursor:pointer;
  }}
  ...
  th {{ background:#1a1a2e; color:#e0b060; padding:8px; text-align:center; position:sticky; top:0; white-space:nowrap; }}
