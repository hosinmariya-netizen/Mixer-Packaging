...
def generate_history_pdf(df, title="سجل History"):
    """توليد PDF من DataFrame باستخدام reportlab مع دعم العربية عبر تعكيس النص"""
    buffer = BytesIO()
    ...
    elements.append(Paragraph(
        f"Total: {len(df)} records  |  Export: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        ParagraphStyle('sub', fontSize=9, alignment=1, textColor=colors.grey)
    ))
    elements.append(Spacer(1, 0.4*cm))

    # ── إعداد بيانات الجدول (نص إنجليزي/أرقام + العربية كما هي)
    cols = list(df.columns)
    header_row = cols  # reportlab يعرض اليونيكود مباشرة
    data_rows = [[str(row[c]) for c in cols] for _, row in df.iterrows()]
    ...
    tbl.setStyle(TableStyle([
        ...
        ('TEXTCOLOR',     (0, 0), (-1, 0), colors.HexColor('#e0b060')),
        ('FONTSIZE',      (0, 0), (-1, 0), 10),
...
renderTable();
            ...
            # ── تصدير PDF عبر طباعة المتصفح (يدعم العربية بالكامل)
            import json as _json
            pdf_cols_json = _json.dumps(cols, ensure_ascii=False)
            pdf_rows_json = _json.dumps(rows_data, ensure_ascii=False)
...
<div id="pdf-export-area" style="display:none">
  <div id="pdf-content">
    <style>
      ...
      #pdf-content {{ direction:rtl; font-family:'Cairo',Arial,sans-serif; }}
      #pdf-content h2 {{ text-align:center; font-size:18px; margin-bottom:4px; }}
      #pdf-content .sub {{ text-align:center; font-size:11px; color:#666; margin-bottom:12px; }}
      #pdf-content table {{ width:100%; border-collapse:collapse; font-size:11px; }}
      #pdf-content th {{ background:#1a1a2e; color:#e0b060; padding:7px 6px; text-align:center; }}
      #pdf-content td {{ padding:5px 6px; border:1px solid #ccc; text-align:center; }}
      #pdf-content tr:nth-child(even) td {{ background:#f5f5f5; }}
      @media print {{
        body * {{ visibility:hidden !important; }}
        #pdf-content, #pdf-content * {{ visibility:visible !important; }}
        #pdf-content {{ position:fixed; top:0; left:0; width:100%; direction:rtl; }}
      }}
    </style>
    <h2>🧸 Baby Sympa — سجل History</h2>
    <p class="sub">عدد السجلات: {total} | تاريخ التصدير: {export_date}</p>
    <table id="pdf-table">
      <thead id="pdf-thead"></thead>
...
<button onclick="doPrint()" style="
  ...
  background:#e0b060; color:#1a1a2e; font-size:15px; font-weight:bold;
  border:none; border-radius:8px; cursor:pointer; font-family:Arial,sans-serif;
">📄 تصدير / طباعة PDF</button>
<script>
var PCOLS = {pdf_cols_json};
var PROWS = {pdf_rows_json};
// بناء الجدول
var thead = document.getElementById('pdf-thead');
thead.innerHTML = '<tr>' + PCOLS.map(function(c){{return '<th>'+c+'</th>';}}).join('') + '</tr>';
var tbody = document.getElementById('pdf-tbody');
tbody.innerHTML = PROWS.map(function(r){{
  return '<tr>' + r.map(function(v){{return '<td>'+v+'</td>';}}).join('') + '</tr>';
}}).join('');
function doPrint() {{
  document.getElementById('pdf-export-area').style.display = 'block';
  window.print();
  setTimeout(function(){{ document.getElementById('pdf-export-area').style.display = 'none'; }}, 1000);
}}
</script>"""
            components.html(pdf_html, height=60, scrolling=False)
