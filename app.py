...
# ── حماية من الخروج بالخطأ عند الضغط على زر الرجوع في الهاتف/المتصفح
if not st.session_state.get("_exit_guard_injected"):
    ...
    (function() {
        try {
            var w = window.parent;
            if (w.__exitGuardInstalled) return;
            w.__exitGuardInstalled = true;
            w.history.pushState({exitGuard: true}, "", w.location.href);
            w.addEventListener('popstate', function(e) {
                var ok = w.confirm("⚠️ هل تريد الخروج من التطبيق؟");
                if (ok) {
                    w.__exitGuardInstalled = false;
                    w.history.go(-1);
                } else {
                    w.history.pushState({exitGuard: true}, "", w.location.href);
                }
    ...
    st.session_state["_exit_guard_injected"] = True
...
if selected_tab == TAB_NAMES[0]:
    ...
    col_confirm, col_save = st.columns(2)
    with col_confirm:
        confirm_out_clicked = st.button("✅ تأكيد الإخراج", use_container_width=True, key="confirm_out_btn")
    with col_save:
        save_out_clicked = st.button("💾 حفظ", use_container_width=True, key="manual_save_out_btn")
    if confirm_out_clicked or save_out_clicked:
...
# ── History
    ...
    if not df_ops.empty:
        ...
        if df_ops_h.empty:
            ...
            # ── تحديد عدد الصفوف المرسلة للجدول لتخفيف الحمل على المتصفح
            total_all = len(df_sum)
            MAX_HIST_ROWS = 300
            if total_all > MAX_HIST_ROWS:
                st.caption(f"🔎 يُعرض أحدث {MAX_HIST_ROWS} سجل من أصل {total_all} — ضيّق البحث (بالمنزل/المنتج/التاريخ) لعرض نتائج أدق.")
                df_sum = df_sum.head(MAX_HIST_ROWS)
