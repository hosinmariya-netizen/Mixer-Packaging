...
with tab4:
    ...
    if not df_ops.empty:
        ...
        df_out = df_ops_f[df_ops_f["النوع"]=="إخراج"].groupby(["المنزل","المنتج","الصنف"])["الكمية"].sum().reset_index()
        df_out.columns = ["المنزل","المنتج","الصنف","المُخرَج"]
        df_in = df_ops_f[df_ops_f["النوع"]=="استلام"].groupby(["المنزل","المنتج","الصنف"])["الكمية"].sum().reset_index()
        df_in.columns = ["المنزل","المنتج","الصنف","المُستلَم"]

        if df_out.empty and df_in.empty:
            st.info("لا توجد نتائج للفلتر المحدد.")
        else:
            if df_out.empty:
                df_balance = df_in.rename(columns={"المُستلَم":"المُستلَم"})
                df_balance["المُخرَج"] = 0
            elif df_in.empty:
                df_balance = df_out.copy()
                df_balance["المُستلَم"] = 0
            else:
                df_balance = df_out.merge(df_in, on=["المنزل","المنتج","الصنف"], how="outer").fillna(0)
